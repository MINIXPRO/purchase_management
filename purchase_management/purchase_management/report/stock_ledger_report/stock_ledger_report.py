...
def execute(filters=None):
	...
	for sle in sl_entries:
		...
		# NEW: Store last entry for each item+warehouse combination
		key = (sle.item_code, sle.warehouse, sle.batch_no or "")
		balance_tracker[key] = sle

		if include_uom:
			conversion_factors.append(item_detail.conversion_factor)

	# NEW: Replace data with only last balance entries (skip opening row)
	if opening_row:
