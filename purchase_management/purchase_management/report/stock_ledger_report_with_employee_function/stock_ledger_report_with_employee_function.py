...
def execute(filters=None):
	...
	# MODIFIED: Dictionary to track last balance for each item+warehouse+employee_function combination
	balance_tracker = {}
	...
	for sle in sl_entries:
		...
		# MODIFIED: Store last entry for each item+warehouse+employee_function combination
		# Get employee_function from sle (it should be fetched from Stock Ledger Entry)
		employee_function = sle.get("employee_function") or ""
		key = (sle.item_code, sle.warehouse, sle.batch_no or "", employee_function)
		balance_tracker[key] = sle

		if include_uom:
			conversion_factors.append(item_detail.conversion_factor)

	# MODIFIED: Replace data with only last balance entries (skip opening row)
	if opening_row:
		final_data = [opening_row]  # Keep opening row
	else:
		final_data = []
	
	# Only show rows where balance is not zero
