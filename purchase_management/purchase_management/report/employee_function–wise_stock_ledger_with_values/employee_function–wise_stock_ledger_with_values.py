...
def execute(filters=None):
	...
	# NEW: Dictionary to track last balance for each item+warehouse combination
	balance_tracker = {}
	...
	for sle in sl_entries:
		...
		# Create balance key for this specific combination
		employee_function = sle.get("employee_function") or ""
		balance_key = (sle.item_code, sle.warehouse, sle.batch_no or "", employee_function)
		
		# Initialize balance for this key if not exists
		if balance_key not in balance_dict:
			balance_dict[balance_key] = {"qty": 0, "value": 0}

		if filters.get("batch_no") or inventory_dimension_filters_applied:
			# Update balance for this specific key
			balance_dict[balance_key]["qty"] += flt(sle.actual_qty, precision)
			balance_dict[balance_key]["value"] += sle.stock_value_difference
			
			# Get current balance for this key
			actual_qty = balance_dict[balance_key]["qty"]
			stock_value = balance_dict[balance_key]["value"]
		...
		# Store in balance tracker
		balance_tracker[balance_key] = sle
